...
Appsim plugin has similar capabilities like the ASTF L7 interpreter instructions. The objective is to simulate L7 applications. This way you get the functionality but without the performance. Each client could have about ~250 active flows (UDP/TCP).

let's start with a an example that simulate SSDP notification (IPv4/UDP/HTTP header)
...
----
        ...
        self.def_ns_plugs = {'arp': {'enable': True},
                            ...
                            'appsim': {
                            "buf_list": [
                                "%v", # replace this with the base64 of the SSDP HTTP text see below  
...
The namespace includes the program that is common for all the clients and each client has this json

[source, python]
.per client json
----

 { "data" : { "name-of-stream" : { "cps": 1.0, "t": "c", "tid": 0, "ipv6": false, "stream": false,"dst" :"239.255.255.250:1900"}} }`

----

in this case there is one stream base of  `tid`  0 (program 0), client side, ipv4, udp, to dest-ip 239.255.255.250:1900 (multicast)

this will create a udp socket evey 1 second and will send one SSDP UDP packet and close the flow


[source, python]
----
c_ssdp_l7 = r`NOTIFY * HTTP/1.1\r
