...
[source,bash]
----
    trex>emu_load_profile -f emu/simple_ipv6.py -t --ns 1 --clients 10 //TODO renamed the file here to simple_ipv6. what is the real name of the file
...
* To debug TRex-EMU packet traces use: `trex-emu  -m | tee /tmp/a.pcap`. It will create a file using the k12 packet format.
...
For STL you can use the Field engine but it is limited with side. We would create a indirect table that could be filled with the information from TRex-EMU. //TODO - What do you mean it is limited with side?
