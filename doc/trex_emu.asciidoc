...
appsim plugin try to mimic the interpreter L7 language instructions from ASTF to simulate application with EMU (instead of ASTF). This way you will get the functionality without the performance. Each client could have about ~250 active flows (UDP/TCP).
...
The ns includes the program that is common for all the clients and each client has this json
