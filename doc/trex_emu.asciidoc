...
The engine we saw until here is very simple and does some simple operation. In case the operation is random it will pick a random value in the domain with uniform probability. But in order to simulate real life cases we need
...
Lastly, let us look at an engine that picks an entry from a list with some predefined distribution.
...
* `dst_mac`: Optional, destination MAC address. If not supplied, the destination MAC address will be the default gateway's MAC address, in case there is one.
* `dst_ipv4`: The collector's IPv4 address. Optional, in case you provide dst_ipv6. One of {dst_ipv4, dst_ipv6} must be provided but not both.
* `dst_ipv6`: The collector's IPv6 address. Optional, in case you provide dst_ipv4. One of {dst_ipv4, dst_ipv6} must be provided but not both.
* `dst_port`: Collector's L4 port, defaults to 4739.
* `src_port`: Exporter's L4 port, defaults to 30334.
...
link:{github_emu_path}/simple_ipfix2.py[emu/simple_ipfix2.py]
...
class Prof1():
        ...
        for i in range(ns_size):
            ...
            for j in range(clients_size):
                c = EMUClientObj(   mac     = mac[j].V(),
                                    ipv4    = ipv4[j].V(),
                                    ipv4_dg = dg.V(),
                                    plugs  = {'ipfix': self.get_init_json()} <1>
                                )
                ns.add_clients(c)
            ns_list.append(ns)
        return EMUProfile(ns = ns_list, def_ns_plugs = self.def_ns_plugs)
    ...
    def get_profile(self, tuneables):
        ...
        assert 0 < args.ns < 65535, 'Namespaces size must be positive!'
...
[source,bash]
----
trex>emu_load_profile -f emu/simple_ipfix2.py
----
...
in the init JSON, but be careful not to create 2 clients with the same domain identifier. Options-Template and Data-Template packets are send with a default rate of 1 pps.

image::images/ipfix1.png[title="IPFix Example",align="left",width={p_width}, link="images/ipfix1.png"]

We can have a closer look at the Options Template packet and see that we have two fields as specified, the first one being a Cisco enterprise field and the second one being the protocol identifier. The scope is 1 as specified in the init JSON.

image::images/ipfix2.png[title="IPFix Options-Template",align="left",width={p_width}, link="images/ipfix2.png"]

Next, we take a closer look at the Data Template packet and see that it has the one IP protocol version.
...
And indeed in the first packet which has two flows (specified in the init JSON with `data_records_num`) we see:
...
emu_ipfix_get_gen_names -      Get IPFix generator names per client.
emu_ipfix_set_data_rate -      Set IPFix generator data rate.
emu_ipfix_show_counters -      Show IPFix data counters data.
----

We can get the generator names:

[source, bash]
----
trex>emu_ipfix_get_gen_names -p 0 --mac 00:00:00:70:00:01
Generator Names

protocolIdentifierGen
ipVersionGen
----


We can get information on one or more generators:

[source, bash]
----
trex>emu_ipfix_get_gen_info -p 0 --mac 00:00:00:70:00:01 -g protocolIdentifierGen ipVersionGen
Ipversiongen

Enabled | Opt. Template | Template Rate (pps) | Data Rate (pps) |   Data Records spec. 
--------+---------------+---------------------+-----------------+----------------------
 True   |     False     |          1          |      0.500      |          5           

|  Data Records calc.  | Template ID  |  Fields |  Engines
+----------------------+-------------+----------+----------
|          5           |     266     |    1     |     1
...
[source, bash]
----
trex>emu_ipfix_show_counters -p 0 --mac 00:00:00:70:00:01  -v
Ipfix Counters

   name     | value | unit | zero  |          help
------------+-------+------+-------+-----------------------
pktTempSent |  42   | pkts | False | Template packets sent.
pktDataSent |  32   | pkts | False | Data packets sent.
...
[source, bash]
----
trex>emu_ipfix_set_data_rate -p 0 --mac 00:00:00:70:00:01 -g ipVersionGen -r 1000
7.19 [ms]
----
...
[source, bash]
----
trex>emu_ipfix_show_counters -p 0 --mac 00:00:00:70:00:01  --zero
Ipfix Counters
...
[source, bash]
----
$:src$ go test -v emu/plugins/icmp -run 3 --monitor 1 > $bdollma/a.pcap
----


==== Transport 

TCP and UDP sockets can be used by a plugin. The Client Transport Context (ctx) will be loaded dynamicly only when the first socket will be used. It working like that to save memory in case there is no use of TCP/UDP (only dot1x)
The TCP logic was ported from BSD c code to native golang (from TRex code )
The socket is working in event driven similar to nodejs and not like golang (micro-threads)

.Socket example 
[source, go]
----
   // ipv4
   ap, err := ctx.Dial("udp", 48.0.0.1:80", cb(), nil)
   //ipv6 
    ap, err := ctx.Dial("tcp", "[2001:db8::3000:1]:80", cb(), nil)
    if err != nil {
        return nil
    }

    // write
     ap.Write([]byte(`{"method" :"request"}`))
----
...
There are utility (rpc) commands to inspect the counters and configuration of the transport context 

Plugin transport_example there is an example of usage of TCP socket 

NOTE: better not to create a socket in the Plugin create beacuse ARP/IPV6 won't have time to complete. Better to delay it 
