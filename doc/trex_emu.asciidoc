...
However, the most common use case is that the Emulation server and the TRex Core Server run on the same machine, as different proccesses. If this indeed is the case, we can use https://en.wikipedia.org/wiki/Inter-process_communication[IPC] as a means of transport. 
