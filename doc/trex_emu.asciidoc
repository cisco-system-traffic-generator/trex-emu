...
- The server supports multiple pools/scopes. Each pool must be passed in a CIDR format. Also, we can specify a list of IPv4 to exclude from the pool in case of static addresses or addresses already in use by the default gateway.
