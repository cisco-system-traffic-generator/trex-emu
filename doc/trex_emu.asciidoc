...
* `dst`: The destination URL. The scheme of the URL is used to determine the exporter type - emu-udp, udp, file, http, or https. If no scheme is specified, emu-udp exporter will be used and the expected string should be of the format host:port. For example, "127.0.0.1:8080" or "[2001:db8::1]:4739". Please refer to 'IPFix Exporters' section below for more information.
* `domain_id`: The observation domain ID as defined in IPFix. If not provided, it will be randomly generated.
* `max_template_records`: An *optional* parameter to limit the number of template records exported by the IPFix plugin after it is loaded. The default value is zero which means no limit is imposed.
* `max_data_records`: An *optional* parameter to limit the number of data records exported by the IPFix plugin after it is loaded. The default value is zero which means no limit is imposed.
* `file_export`: An *optional* JSON object used to configure the file exporters. Please refer to the 'IPFix exporters' section below for more information.
...
==== IPFix Exporters
The IPFix plugin supports several ways to send the generated template and data records to the collector, namely - *exporters*. Exporters are defined and configured per client as part of its initialization JSON. The type of exporter to use and destination URL are specified by the "dst" field in the init JSON as explained below.

[NOTE]
=====================================================================
In order to use the kernel-level exporters (udp, file, http, and https) - the emu-server must run in kernel mode as follows:
[source, bash]
----
bin$ ./trex-emu -k
----
...
* `emu-udp`:
In this case flow records are sent using the TREX-EMU UDP transport where the UDP packets are generated in the application layer. The IPfix records are aggregated up to MTU size and sent to the specified IP:PORT destination. There are two ways to define it using the "dst" field:
...
* `udp`:
Flow records are sent using GoLang UDP sockets library utilizing the OS kernel. Records are aggregated up to MTU size and sent to the specified IP:PORT destination. 
"dst" field should be specified as follows - `"udp://host:port"`.
Example: “udp://10.56.216.158:8080”

* `file`:
Flow records are stored in files in a user-defined directory. The generated records are aggregated in a single file up to a max size and for max interval duration. Once the file aggregation reaches either of these limits, it will be ‘rotated’ – closed and compressed with a unique name in the same directory.
...
dir/name can be also specified explicitly in the `file_export` object.

* `http/https`:
This exporter is based on the file exporter to temporarily aggregate flow records in files.
Once a file is rotated, a POST HTTP or HTTPS post request will be created to the user-defined URL.
The temporary file will be deleted after the porst request transaction is completed (either with success or failure).
...
Rotated files will be sent to destination URL using HTTP/HTTPS.

The `file_export` optional object in the JSON may be used to configure the file exporters (file, http, or https). Also, all fields of `file_export` are optional. If a given field is not specified, its default value will be used.
[source, json]
.file_export example
----
    "file_export": {
...
* `dir` (default: empty string):  The path in which export files are created. If empty string (the default), will use the OS temp directory (/tmp).
The exporter of each client will have a separate unique folder under dir path with the following name – `<base>_<mac>.<ext>`
Example: fnf_agg_000000700003.ipfix. As an example, suppose the user’s directory is ‘/scratch/myproject’, name is ‘fnf_agg.ipfix’ and we have 2 clients - 01:01:01:01:01:01 and 02:02:02:02:02:02.
Files will be created in two separate folders - 
`/scratch/myproject/fnf_agg_010101010101.ipfix/fnf_agg_1.20220603084739-20220603084839.ipfix.gz` and `/scratch/myproject/fnf_agg_020202020202.ipfix/fnf_agg_1.20220603084739-20220603084839.ipfix.gz`
...
* `max_posts` (default: 0): For HTTP/HTTPs - limit the maximum number of posts requests to send. Once the exporter reached this limit no further posts will be created.
