...
func LogTimeFormatted(logString string, logLevel LogLevel) {
	fmt.Println(time.Now().Format(time.RFC3339) + fmt.Sprintf(" [%s] -> %s ", logLevel, logString))
}
...
func NewPPPNs(ctx *core.PluginCtx, initJSON []byte) *core.PluginBase {
	...
	LogTimeFormatted(">> NewPPPNs >> Plugin PPP Ns", INFO)
...
func (o *PluginPPPNs) OnRemove(ctx *core.PluginCtx) {
	LogTimeFormatted(">> OnRemove >> Plugin PPP Ns", INFO)
}

func (o *PluginPPPNs) OnEvent(msg string, a, b interface{}) {
	LogTimeFormatted(">> OnEvent >> Plugin PPP Ns", INFO)
}

func (o *PluginPPPNs) SetTruncated() {
	LogTimeFormatted(">> SetTruncated >> Plugin PPP Ns", INFO)
}
