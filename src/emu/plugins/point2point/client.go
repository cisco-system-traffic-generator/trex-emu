...
func (o *PluginPPPClient) OnCreate() {
	...
	if dot1qLayer := tmp.Layer(layers.LayerTypeDot1Q); dot1qLayer != nil {
		...
		LogTimeFormatted(INFO,
			">> OnCreate >> VLAN %d created PPPClient at Mac -> %s",
			dot1q.VLANIdentifier, clientMac)
	} else {
		LogTimeFormatted(INFO, ">> OnCreate >> VLAN Untagged created PPPClient at Mac -> %s",
			clientMac)
	}

	if clientMac.IsZero() {
		msg := "Error during PPP Client creation : provided Mac Address is \"Zero\""
		LogTimeFormatted(CRITICAL, msg)
		panic(msg)
	}
...
func (o *PluginPPPClient) sendPADI() {
	...
	o.Tctx.Veth.SendBuffer(false, o.Client, padi, false)
}
...
func (o *PluginPPPClient) sendPADR() {
	...
	o.restartTimer(o.maxTimerRetransmitSec)
	o.Tctx.Veth.SendBuffer(false, o.Client, padr, false)
}
...
func (o *PluginPPPClient) sendPADT() {
	...
	if dot1qLayer := tmp.Layer(layers.LayerTypeDot1Q); dot1qLayer != nil {
		dot1q, _ := dot1qLayer.(*layers.Dot1Q)
		LogTimeFormatted(INFO, ">> sendPADT >> VLAN %d Send PADT Message from Mac -> %s",
			dot1q.VLANIdentifier, o.Client.Mac)
	} else {
		LogTimeFormatted(INFO, ">> sendPADT >> Send PADT Message from Mac -> %s",
			o.Client.Mac)
	}
	...
	o.restartTimer(o.maxTimerRetransmitSec)
	o.Tctx.Veth.SendBuffer(false, o.Client, padt, false)
	o.padtSent += 1
}
...
func (o *PluginPPPClient) OnEvent(msg string, a, b interface{}) {
	clientMac := o.Client.GetInfo().Mac
	LogTimeFormatted(INFO, ">> OnEvent >> Created PPP Client at Mac -> %s",
		clientMac)
}
...
func (o *PluginPPPClient) OnRemove(ctx *core.PluginCtx) {
	...
	clientMac := o.Client.GetInfo().Mac
	LogTimeFormatted(INFO, ">> OnRemove >> Destroyed PPP Client at Mac -> %s",
		clientMac)
...
func (o *PluginPPPClient) HandleRxPPPPacket(ps *core.ParserPacketState) int {
	...
	case PPPStateLinkUp:
		...
		// Parsing PPP IPCP pkts
		if o.isPPP(ps, layers.PPPTypeIPCP) {
			LogTimeFormatted(INFO, ">> PPPStateLinkUp >> IPCP on Mac %v -> Ack with negotiated IP %v",
				o.Client.Mac, o.negClientIP)
			o.handleIPCPNegotiation(ps)
		} else if o.isLCP(ps, layers.LCPTypeEchoRequest) {
			o.answerLCPEcho(ps)
...
func (o *PluginPPPClient) answerLCPTerminate(ps *core.ParserPacketState) {
	...
	LogTimeFormatted(WARNING, ">> LCP Terminate Request >> Received on Mac %v",
		o.Client.Mac)
	...
	for _, option := range lcp.Options {
		if option.Type == layers.LCPOptionQualityProtocol {
			LogTimeFormatted(WARNING, ">> answerLCPTerminate >> LCP Terminate Request on Mac %v for Quality-Protocol",
				o.Client.Mac)
		}
	}
...
func (o *PluginPPPClient) sendLCPMsg(lcpCode layers.LCPType, identifier uint8) {
	...
	o.restartTimer(o.minTimerRetransmitSec)
	o.Tctx.Veth.SendBuffer(false, o.Client, lcpMsg, false)
}
...
func (o *PluginPPPClient) sendPAPReq() {
	...
	o.restartTimer(o.maxTimerRetransmitSec)
	o.Tctx.Veth.SendBuffer(false, o.Client, papReq, false)
}
...
func (o *PluginPPPClient) handlePAPRes(ps *core.ParserPacketState) {
	...
	if code := pap.Code; code == layers.PAPTypeAuthAck {
		...
		LogTimeFormatted(WARNING, ">> PluginPPPClient.handlePAPRes >> Unexpected PAP returned code [%d] on Mac %v",
			code, o.Client.Mac)
	}
}
...
func (o *PluginPPPClient) handleIPCPNegotiation(ps *core.ParserPacketState) {
	...
	case layers.IPCPTypeConfigurationAck:
		...
		if bytes.Equal(o.negClientIP[:], ipcp.GetProposedIPAddress()) {
			LogTimeFormatted(INFO, "Client on Mac %v has IP %v with PPP Id %04X !",
				o.Client.Mac, o.negClientIP, o.pppSessionID)
...
func (o *PluginPPPClient) sendIPCPMsg(ipcpCode2Send layers.IPCPType, identifier uint8) {
	...
	o.restartTimer(o.maxTimerRetransmitSec)
	o.Tctx.Veth.SendBuffer(false, o.Client, ipcpMsg, false)
}
